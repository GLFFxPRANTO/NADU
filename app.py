...
def get_jwt():
    """
    JWT üretici servisine istek atarak token elde eder.
    """
    url = JWT_GEN_URL.format(uid=JWT_UID, password=JWT_PASS)
    try:
        response = requests.get(url, timeout=10)
        if response.status_code == 200:
            token_data = response.json()
            # Token, "token" anahtarı altında geliyor
            return token_data.get("token", token_data)
        else:
            print(f"JWT oluşturma hatası, status code: {response.status_code}")
            return None
    except Exception as e:
        print("JWT alma hatası:", e)
        return None
